...
export const HellenisticCampaign = () => {
  ...
  // Pick first 2 slides — main + secondary
  const main = slides[0];
  ...
  return (
    <section
      ...
      className={`relative overflow-hidden bg-[#F8F5EE] text-[#1F1B14] lx-reveal ${visible ? "is-visible" : ""}`}
    >
      <div className="ix-grain pointer-events-none absolute inset-0" />

      {/* Top editorial bar */}
      <div className="relative z-10 mx-auto flex max-w-[1800px] items-center justify-between border-b border-[#1F1B14]/8 px-6 py-6 sm:px-12">
        <div className="flex items-center gap-3 text-[10px] uppercase tracking-[0.5em] text-[#1F1B14]/55">
          <span className="h-1 w-1 rounded-full bg-[#B89968]" />
          {t("hellenistic.label")}
        </div>
        <div className="hidden text-[10px] uppercase tracking-[0.45em] text-[#1F1B14]/55 md:block">
      ...
      <div className="relative z-10 mx-auto grid max-w-[1800px] grid-cols-1 px-6 pb-24 pt-16 sm:px-12 sm:pb-32 sm:pt-24 lg:grid-cols-[1fr_1.45fr] lg:gap-20 lg:pb-40 lg:pt-32">
          ...
          <div>
            ...
            <div
              ...
              style={{
                ...
                background: "linear-gradient(180deg, #1F1B14 0%, #D6C8A8 100%)",
            ...
            <div className="mt-6 lg:mt-10">
              <div className="text-[10px] uppercase tracking-[0.5em] text-[#B89968]">
                {pickText(main?.kicker, "EDITION 01 · COMING SOON")}
              </div>
              <h2
                className="font-display mt-5 text-[#1F1B14]"
                style={{
                  fontSize: "clamp(48px, 6.5vw, 116px)",
                  lineHeight: 0.9,
              ...
              >
                ...
                <br />
                <span className="ix-gold-text">{pickText(secondary?.title, "Ascension")}</span>
              </h2>

              <p className="mt-8 max-w-md text-[15px] leading-[1.9] text-[#3A352B]/85">
                {pickText(main?.subtitle, t("hellenistic.body"))}
              </p>

              {/* Specs row */}
              <div className="mt-12 grid max-w-md grid-cols-3 gap-6 border-t border-[#1F1B14]/10 pt-7">
              ...
              <Link
                ...
                className="ix-btn mt-12 inline-flex"
              >
                {t("hellenistic.cta")}
        ...
        <div className="relative order-1 lg:order-2">
          ...
          {/* Main image (large, vertical) */}
          {main?.image_url && (
            <div className="relative aspect-[4/5] overflow-hidden bg-[#E8DCC4] sm:aspect-[3/4] lg:aspect-[4/5]">
              <img
                src={main.image_url}
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#1F1B14]/40 via-transparent to-transparent" />

              {/* Bottom caption */}
              <div className="absolute bottom-5 left-5 right-5 flex items-end justify-between gap-4">
                <div className="ix-glass px-4 py-2.5">
                  <div className="text-[9px] uppercase tracking-[0.35em] text-[#1F1B14]/65">
                    {pickText(main.kicker, "Featured piece")}
                  </div>
                  <div className="font-display mt-0.5 text-base uppercase tracking-[0.08em] text-[#1F1B14]">
                    {pickText(main.title, "Hellenistic Ascension")}
                  </div>
                </div>
                <div className="hidden text-right text-[#F8F5EE] sm:block">
                  <div className="text-[10px] uppercase tracking-[0.35em] text-[#F8F5EE]/75">
                    {t("hellenistic.from")}
                  </div>
          ...
          {/* Floating secondary image (overlapping bottom-right corner of main) */}
          {secondary?.image_url && secondary.image_url !== main?.image_url && (
            <div className="absolute -bottom-10 right-0 hidden aspect-[3/4] w-[38%] overflow-hidden border-8 border-[#F8F5EE] shadow-2xl lg:block">
              <img
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#1F1B14]/55 via-transparent to-transparent" />
              <div className="absolute bottom-3 left-3 right-3">
                <div className="text-[9px] uppercase tracking-[0.4em] text-[#F8F5EE]/85">
                  {pickText(secondary.kicker, "II · DROP")}
                </div>
                <div className="font-display mt-1 text-base uppercase text-[#F8F5EE]">
          ...
          {/* Gold hairline below */}
          <div className="ix-gold-line mt-6 max-w-[60%]" />
        </div>
      </div>
    </section>
  );
...
const Spec = ({ label, value }) => (
  <div>
    <div className="text-[9px] uppercase tracking-[0.35em] text-[#1F1B14]/45">{label}</div>
    <div className="font-display mt-2 text-sm uppercase tracking-[0.06em] text-[#1F1B14]">{value}</div>
  </div>
);
