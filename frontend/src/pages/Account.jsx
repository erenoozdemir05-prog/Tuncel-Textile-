  ...
  if (loading || !user) {
    return (
      <div className="mx-auto max-w-[1400px] px-5 py-32 text-center sm:px-8">
        <p className="text-[12px] uppercase tracking-[0.25em] text-neutral-500">{t("account_page.loading")}</p>
      </div>
    );
  }
  ...
  return (
    <div data-testid="account-page" className="mx-auto max-w-[1400px] px-5 sm:px-8">
      <section className="flex flex-wrap items-end justify-between gap-6 border-b border-black/10 py-12">
        <div>
          <div className="text-[11px] uppercase tracking-[0.3em] text-neutral-500">{t("account_page.kicker")}</div>
          <h1 className="font-display mt-2 text-6xl uppercase leading-none tracking-[0.02em] sm:text-8xl">
            {t("account_page.greet").replace("{name}", user.name?.split(" ")[0] || t("account_page.there"))}
          </h1>
        ...
        >
          <LogOut className="h-3.5 w-3.5" /> {t("account_page.sign_out")}
        </button>
      </section>

      <section className="py-12">
        <div className="text-[11px] uppercase tracking-[0.3em] text-neutral-500">{t("account_page.history_kicker")}</div>
        <h2 className="font-display mt-2 text-4xl uppercase tracking-[0.04em] sm:text-5xl">{t("account_page.history_title")}</h2>
              ...
              <div key={o.session_id} data-testid={`order-row-${o.session_id}`} className="grid grid-cols-1 gap-4 py-6 sm:grid-cols-[1fr_auto_auto] sm:items-center">
                <div>
                  ...
                  <div className="font-display mt-1 text-2xl uppercase tracking-[0.04em]">
                    {t("account_page.order_label")} {o.session_id.slice(-8).toUpperCase()}
                  </div>
                  <div className="mt-1 text-sm text-neutral-600">
                    {(o.items || []).reduce((s, i) => s + (i.quantity || 1), 0)} {t("account_page.items_word")} · {(o.metadata?.items || "").slice(0, 80)}
                  </div>
                </div>
